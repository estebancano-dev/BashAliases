	...
	do
		case $opt in
			...
			"custom ext")
				...
				;;
			"custom ext dicc 1-5")
				python ~/tools/dirsearch/dirsearch.py -t 50 -f -e $2 -w ~/tools/__diccionarios/1-5.txt -u $1 $3
				break
				;;
...
customdictfromurl(){
	...
	if [[ $1 =~ $validate ]]; then 
		...
		echo $1 | gau -t 10 -b ttf,woff,svg,png,jpg,ico,woff2,jpeg | sed -E '/(\.ttf$|\.woff$|\.svg$|\.png$|\.jpeg$|\.jpg$|\.ico$|\.woff2$|\.jpeg$)/d' | fff | wordlistgen -fq >> ~/tools/__diccionarios/dictionary$1.txt
