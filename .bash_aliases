...
massdns(){
	~/tools/massdns -r $1 -w ~/tools/subd$1results.txt
}
...
reinstall(){
	cd ~/
	curl -H 'Cache-Control: no-cache' https://raw.githubusercontent.com/estebancano-dev/BashAliases/master/.bash_aliases > .bash_aliases
	reload
}
