...
subdomains(){
	...
	if [[ -f 4nmapips$1.txt && -s 4nmapips$1.txt ]]; then
		...
		nmap -sS -Pn -T5 --top-ports 2000 --script vuln -iL 4nmapips$1.txt > 7nmapvuln$1.txt < /dev/null 2>&1
	fi
	...
	do
		case $opt in
			...
			"no ext dicc")
				...
				;;
			"no ext file")
				python3 ~/tools/dirsearch/dirsearch.py -t 50 --random-agents -x 301,302,400 --plain-text-report=PLAINTEXTOUTPUTFILE -f -L $1 -e , > $1dirsearch.txt
				break
				;;
			"all ext")
